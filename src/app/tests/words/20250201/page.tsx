...
export default function Page() {
  ...
  return (
    <main ref={containerRef} className="min-h-screen w-full bg-black flex flex-col items-start justify-start p-8 relative">
      ...
      <div className="flex flex-col">
        {words.map((word, idx) => (
          <p key={idx} className={`${inter.className} text-white font-extrabold transition-opacity duration-[2000ms] opacity-${visibleIndices[idx] ? '100' : '0'}`} style={{ fontSize: '5vh', lineHeight: '1.1em' }}>
            {word}
          </p>
        ))}

        {discipline && showDiscipline && (
          <p className={`${randomColor} ${inter.className} font-extrabold`} style={{ fontSize: '5vh', lineHeight: '1.1em' }}>
            {discipline}
          </p>
        )}
      </div>
