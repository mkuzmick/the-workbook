...
const Stars = () => {
  // Compute the star data only once on mount
  const starData = useMemo(
    () =>
      Array.from({ length: 80 }).map((_, i) => ({
        ...
        move: Math.random() * 4 - 2, // pixel offset for animation
        opacity: Math.random(), // random opacity value
        duration: Math.random() * 10 + 20, // random duration for animation
      })),
    []
  );
